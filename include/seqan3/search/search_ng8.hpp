...
enum class Dir_ng8 : uint8_t { Left, Right };
template <typename T>
struct Block_ng8 {
	T       rank;
	uint8_t l;
...
struct Search_ng8 {
	...
	Search_ng8(index_t const& _index, search_scheme_t const& _search, size_t _qidx, delegate_t const& _delegate)
		: index     {_index}
		, search    {_search}
		, qidx      {_qidx}
		, delegate  {_delegate}
	{
		...
		using _Info = Info_ng8<true, true, true, 'M'>;
		search_next<_Info, _Info>(cur, 0, blockIter);
	...
	void search_next(cursor_t const& cur, int e, BlockIter blockIter) const noexcept {
		...
		if (blockIter->dir == Dir_ng8::Right) {
	...
	void search_next_dir(cursor_t const& cur, int e, BlockIter blockIter) const noexcept {
		...
		using OnMatchL      = std::conditional_t<Right, LInfo, Info_ng8<true, true, true, 'M'>>;
		using OnMatchR      = std::conditional_t<Right, Info_ng8<true, true, true, 'M'>, RInfo>;
		using OnSubstituteL = std::conditional_t<Right, LInfo, Info_ng8<true, false, false, 'S'>>;
		using OnSubstituteR = std::conditional_t<Right, Info_ng8<true, false, false, 'S'>, RInfo>;
		using OnDeletionL   = std::conditional_t<Right, LInfo, Info_ng8<true, true, false, 'D'>>;
		using OnDeletionR   = std::conditional_t<Right, Info_ng8<true, true, false, 'D'>, RInfo>;
		using OnInsertionL  = std::conditional_t<Right, LInfo, Info_ng8<true, false, true, 'I'>>;
		using OnInsertionR  = std::conditional_t<Right, Info_ng8<true, false, true, 'I'>, RInfo>;
...
void search_ng8(index_t const & index, queries_t && queries, uint8_t _max_error, search_schemes_t const & search_scheme, delegate_t && delegate)
{
    ...
    std::vector<std::vector<Block_ng8<size_t>>> search_scheme2;
    for (auto s : search_scheme) {
        std::vector<Block_ng8<size_t>> search2;
        for (size_t i{0}; i < s.pi.size(); ++i) {
            auto dir = [&]() {
                if (i == 0) {
                    return s.pi[i] < s.pi[i+1]?Dir_ng8::Right:Dir_ng8::Left;
                } else {
                    return s.pi[i-1] < s.pi[i]?Dir_ng8::Right:Dir_ng8::Left;
                }
            }();
            search2.emplace_back(Block_ng8<size_t>{{}, s.l[i], s.u[i], dir});
        }
    ...
    for (size_t i{0}; i < queries.size(); ++i) {
        ...
        for (size_t j{0}; j < search_scheme.size(); ++j) {
            ...
            Search_ng8{index, search, i, internal_delegate};
        }
    }
