...
enum class Dir_ng9 : uint8_t { Left, Right };
template <typename T>
struct Block_ng9 {
	T       rank;
	uint8_t l;
...
template <bool fast_lex, typename index_t, typename search_scheme_t, typename delegate_t>
struct Search_ng9 {
	...
	Search_ng9(index_t const& _index, search_scheme_t const& _search, size_t _qidx, delegate_t const& _delegate)
		: index     {_index}
		, search    {_search}
		, qidx      {_qidx}
		, delegate  {_delegate}
	...
	auto extend(cursor_t const& cur, Dir_ng9 dir, index_rank_type rank) const noexcept {
		if (dir == Dir_ng9::Right) {
	...
	template <bool Right>
	auto extend(cursor_t const& cur, index_rank_type rank) const noexcept {
		if constexpr (Right) {
			return cur.extend_right(rank);
		} else {
			return cur.extend_left(rank);
	...
	void extend_cb(cursor_t const& cur, Dir_ng9 dir, CB const& cb) const noexcept {
		if (dir == Dir_ng9::Right) {
	...
	void extend_cb(cursor_t const& cur, CB const& cb) const noexcept {
		if constexpr (Right) {
			cur.extend_right_cb(cb);
		} else {
			cur.extend_left_cb(cb);
...
//		if (blockIter->dir == Dir_ng9::Right) {
			search_next_dir<true>(cur, e, blockIter);
/*		} else {
			search_next_dir<false>(cur, e, blockIter);
		}*/
	}
	...
	void search_next_dir(cursor_t const& cur, int e, BlockIter blockIter) const noexcept {
		...
		if (blockIter->l <= e+1 and e+1 <= blockIter->u) {
			if constexpr (fast_lex) {
				...
				extend_cb(cur, blockIter->dir, [&](auto c, auto newCur) {
					if (c > index_sigma) return;
					cursors[c] = newCur;
				});
				for (index_rank_type i{1}; i < rank; ++i) {
					search_next(cursors[i], e+1, blockIter+1); // as substitution
				}
...
template <bool fast_lex, typename index_t, typename queries_t, typename search_schemes_t, typename delegate_t>
void search_ng9(index_t const & index, queries_t && queries, uint8_t _max_error, search_schemes_t const & search_scheme, delegate_t && delegate)
{
    ...
    std::vector<std::vector<Block_ng9<size_t>>> search_scheme2;
    for (auto s : search_scheme) {
        std::vector<Block_ng9<size_t>> search2;
        for (size_t i{0}; i < s.pi.size(); ++i) {
            auto dir = [&]() {
                if (i == 0) {
                    return s.pi[i] < s.pi[i+1]?Dir_ng9::Right:Dir_ng9::Left;
                } else {
                    return s.pi[i-1] < s.pi[i]?Dir_ng9::Right:Dir_ng9::Left;
                }
            }();
            search2.emplace_back(Block_ng9<size_t>{{}, s.l[i], s.u[i], dir});
        }
    ...
    for (size_t i{0}; i < queries.size(); ++i) {
        ...
        for (size_t j{0}; j < search_scheme.size(); ++j) {
            ...
            Search_ng9<fast_lex, index_t, std::decay_t<decltype(search)>, std::decay_t<decltype(internal_delegate)>>{index, search, i, internal_delegate};
        }
    }
